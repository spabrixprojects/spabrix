...
export default function Navbar() {
  ...
  return (
    <motion.nav
      initial={{ y: -80 }}
      animate={{ y: 0 }}
      className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#0d1117]/95 shadow-lg backdrop-blur-md border-b border-gray-800' : 'bg-transparent'
        }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-14">
        <div className="flex justify-between  h-16 items-center">
          ...
          <Link href="/" aria-label="Fozecode Home" >
            <Image src="/spabrixl.png" width={180} height={180} alt="Fozecodelogo" />
          ...
          <div className="hidden md:flex space-x-8">
            {menuItems.map(({ name, href }) => (
              <Link
                key={name}
                href={href}
                className={`transition-colors text-gray-300 hover:text-cyan-400 ${pathname === href ? 'text-white font-semibold' : ''
                  }`}
              >
                {name}
              </Link>
